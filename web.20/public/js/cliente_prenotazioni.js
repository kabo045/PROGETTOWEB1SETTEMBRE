  ...
  try {
    ...
    listaPrenotazioni.innerHTML = pren.map(p => {
      ...
      <div class="col-md-6 col-lg-4">
        <div class="card shadow h-100 prenotazione-card border-0 animate__animated animate__fadeIn" style="overflow:hidden;">
          <div class="card-body pb-2">
            <h5 class="card-title mb-1">${p.location_name}</h5>
            <span class="badge bg-primary">${p.space_type}</span>
            ...
            <p class="mb-1"><strong>Pagamento:</strong> ${
              p.amount ? `€${p.amount.toFixed(2)} <span class="badge bg-${p.payment_status === 'completato' ? 'success' : 'danger'}">${p.payment_status}</span>` : '—'
            }</p>
            ${
              !disableCancel
...
window.cancellaPrenotazione = function(id) {
  ...
  confirmToast.innerHTML = `
    <div class="p-3 pb-2 d-flex flex-column gap-1">
      <div class="d-flex align-items-center mb-1">
        <span class="fs-2 text-warning me-2 animate__animated animate__tada"><i class="bi bi-exclamation-triangle-fill"></i></span>
  ...
  // Chiudi dopo 10s se non risponde
